...
def process_mats(input_file,
    ...
    for idx, event_entry in input_mats.iterrows():
        ...
        if not event_id in known_event_list: 
            continue
...
def import_event_list_from_rMATS_output(input_file, event_type): 
    # import mats file
    input_mats = pd.read_csv(input_file, sep='\t', index_col=0)
    ...
    return(set(event_list))  # return set for fast lookup 
...
if __name__ == "__main__":
    ...
    for event_type in event_type_list:
        print("Importing Known Events for %s - %s" % (study, event_type))

        # import known files  
        anno_file = "%s/fromGTF.%s.txt" % (input_dir, event_type)
        noveljc_anno_file = "%s/fromGTF.novelJunction.%s.txt" % (input_dir, event_type)
        novelss_anno_file = "%s/fromGTF.novelSpliceSite.%s.txt" % (input_dir, event_type)

        all_event_list = import_event_list_from_rMATS_output(anno_file, event_type)
        noveljc_event_list = import_event_list_from_rMATS_output(noveljc_anno_file, event_type)
        novelss_event_list = import_event_list_from_rMATS_output(novelss_anno_file, event_type)

        # extract known events 
        known_list = []
        for event in all_event_list:
            if not event in noveljc_event_list and event not in novelss_event_list:
                known_list.append(event)
        known_list = set(known_list)
        ...
        output_file = process_mats(input_mats_file, event_type, output_dir, study, known_list, one_group_mode)
